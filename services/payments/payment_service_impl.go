...
func (service *PaymentServiceImpl) ProcessPayment(ctx context.Context, input web.PaymentNotificationInput) error {
	...
	if checkResp != nil {
		...
		if checkResp.TransactionStatus == "capture" {
			if checkResp.FraudStatus == "challenge" {
				status = "challenge" // Transaksi masuk, tapi mencurigakan (perlu diapprove manual di dashboard midtrans)
			} else if checkResp.FraudStatus == "accept" {
				status = "paid" // Tansaksi Aman & Sukses
			}
		} else if checkResp.TransactionStatus == "settlement" {
			// Case 2: Settlement (Transfer Bank, E-Wallet, dll)
			// Ini artinya uang sudah dipastikan masuk (Settled).
			status = "paid" 
		} else if checkResp.TransactionStatus == "deny" || 
		checkResp.TransactionStatus == "expire" || 
		checkResp.TransactionStatus == "cancel" {
		...
		if status != "pending" {
			err := service.orderRepo.UpdateStatus(ctx, service.DB, input.OrderID, status)
			if err != nil {
				return err
			}
		}
